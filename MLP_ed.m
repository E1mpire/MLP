...
%班级：2002011
%专业：电子信息工程
%设计前馈神经网络，辨别MINST数字奇偶
%输入层：28*28个神经元，隐含层：100个神经元，输出层：2个神经元
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clear,clc
%%生成数据点
...
for i=1:n
    if mod(train_label(i),2)==0  % 如果偶数那么第一个就为1，反之第二个为1
        trainlabel(1,i)=1;
    else
        trainlabel(2,i)=1;
    end
...
testlabel=zeros(2,t);
for i=1:t
   if mod(test_label(i),2)==0  % 如果偶数那么第一个就为1，反之第二个为1
        testlabel(1,i)=1;
    else
        testlabel(2,i)=1;
    end
...
%{
w1=randn(cell,input_layer);%第一层神经网络的权值
w2=randn(output_layer,cell);%第二层神经网络的权值
b1=randn(cell,1);%第一层神经网络的偏置
b2=randn(output_layer,1);%第二层神经网络的偏置
...
%}
load('init_ed')
w1=w1o;w2=w2o;b1=b1o;b2=b2o;%读取挑好的初值
...
e=sum(-trainlabel(:,1).*log(a2));             %交叉熵损失函数
%%%%%%%%%%%%%%%%%%%%%%
    ...
    for i=2:n
        ...
        n2=w2*a1+b2;
        a2=softmax(n2);                     %第二层激活函数softmax
        e=sum(-trainlabel(:,i).*log(a2)); %交叉熵损失函数
...
for i=1:t
    ...
    if mod(i,20)==0
        fprintf('当前数据为%d,判断数据为%d （1为偶数0为奇数）\n',posl-1,posa-1)
    end
end
